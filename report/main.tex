...
Stage      : ivf_pq_basic
N          : 1000000
dim        : 128  k : 10  queries : 100
nlist      : 256  nprobe : 32  M : 32
build_ms   : 341857
query_ms   : 534.484
QPS        : 187.096
recall@k   : 0.72
...
N          : 1000000
dim        : 128  k : 10  queries : 100
nlist      : 256  nprobe : 32  M : 32  n_streams : 8
build_ms   : 143685
query_ms   : 363.057
QPS        : 275.439
recall@k   : 0.72
...
cmake ..                              # configures all 5 targets
make ivf_pq_optimized                 # build Stage 5 (or any target name)
\end{lstlisting}

\subsection*{Execution}
SIFT1M data files (\texttt{sift\_base.fvecs}, \texttt{sift\_query.fvecs},
\texttt{sift\_groundtruth.ivecs}) must be present in \texttt{data/sift/}.
Usage: \code{<binary> <N> <dim> <k> <Q>}.

\begin{lstlisting}[style=terminal]
cd ~/parallel-vector-database
./build/baseline_knn      1000000 128 10 100   # Stage 1 (ground truth)
./build/naive_knn         1000000 128 10 100   # Stage 2
./build/optimized_knn     1000000 128 10 100   # Stage 3
./build/ivf_pq_basic      1000000 128 10 100   # Stage 4
./build/ivf_pq_optimized  1000000 128 10 100   # Stage 5

# Or submit via SLURM (instruction partition, 45-min limit):
sbatch slurm/ivf_pq_basic.slurm
\end{lstlisting}

Each binary appends one CSV row to the corresponding file in
\texttt{benchmarks/results/} and prints all metrics to stdout.
Stage~1 additionally writes \texttt{benchmarks/results/ground\_truth.bin}
(used by Stages 2--5 for recall computation).
